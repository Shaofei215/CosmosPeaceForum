"""Agent 服务统一日志、上下文与 HTTP 访问日志。

Management、Scheduler、外部工具网关与内部 HTTP 服务均通过本模块把同一条
``LogRecord`` 输出到控制台、结构化 JSONL 文件和管理端终端缓冲。
"""
...
_LOG_CONTEXT: ContextVar[dict[str, Any]] = ContextVar("agents_log_context", default={})
...
def get_request_id() -> str | None:
    """返回当前请求、后台任务或 Agent 会话的关联 ID。"""

    value = _LOG_CONTEXT.get().get("request_id")
    return str(value) if value is not None else None


def get_outbound_request_headers() -> dict[str, str]:
    """构造供内部或下游 HTTP 调用传播的关联请求头。"""
...
def _component_for_logger(logger_name: str) -> str:
    """根据 logger 命名空间归类 Agent 服务组件。"""

    if logger_name.startswith("alembic"):
        return "migration"
    if logger_name.startswith("agents.management"):
        return "management"
    if logger_name.startswith("agents.external_access"):
...
class HybridRotatingFileHandler(logging.Handler):
    ...
    def _should_rollover(self, encoded_size: int) -> bool:
        ...
        if self._now_factory().date() != self._opened_date:
            return True
        try:
    ...
    def cleanup(self, *, now: datetime | None = None) -> None:
        ...
        archives = sorted(self.log_dir.glob("runtime.*.jsonl"), key=lambda path: path.stat().st_mtime)
        ...
        for path in archives:
            ...
            if path.exists():
                ...
                path.unlink(missing_ok=True)
                total_size -= size
        self._archive_total_bytes = sum(path.stat().st_size for path in archives if path.exists())
...
def restore_agents_loggers() -> None:
    """恢复可能被迁移配置禁用的 Agent 命名空间 logger。"""

    for name, logger_obj in logging.Logger.manager.loggerDict.items():
        if isinstance(logger_obj, logging.Logger) and (name == "agents" or name.startswith("agents.")):
...
class AccessLogMiddleware:
    """为 Agent HTTP 请求建立关联上下文并输出统一访问日志。"""
    ...
    def __init__(
        ...
        *,
        ...
        self.logger = logging.getLogger("agents.access")
        ...
        try:
            ...
            self.logger.exception(
                ...
                extra={
                    ...
                    "http": self._http_fields(method, route, status_code, elapsed_ms, client_ip, headers),
                },
            )
            raise
        finally:
