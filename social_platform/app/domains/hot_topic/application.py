...
- 生成 5 到 10 条适合公开展示的候选热点事件、话题条目。综合站内讨论热点、外部搜索的最新的各领域时事。历史生成记录仅用于生成后续、参照风格等等，而不是用于再把历史中出现多次的内容再复述一遍。
- 每条都必须来自上下文、站内搜索结果或可验证的外部搜索结果，不得补写没有依据的事实。
- 标题、摘要和搜索词都只描述事件本身，不评价热度、排名、趋势、爆火程度或推荐理由。
- 总结社区时，请不要总结讨论度高的却鸡毛蒜皮的无意义的低价值的小事，榜单仍主要以深度的、有价值的热门时事讨论为主。
- 搜索最新时事时，请将`time_range`参数设为`day`以搜索当天的最新消息。

## 输出字段

- `title`：简洁中文标题，聚焦一个具体事件或讨论主题，不写“热榜”“热门”“第几名”“最受关注”等与事件无关的表达。
- `summary`：一句话说明事件核心信息，不超过 150 个中文字符；不要解释入选原因、讨论量、排序依据或热度变化。
- `search_query`：只能是一个高度简单且可以高度代表该条目、关键词搜索命中率尽可能高的搜索关键词或一个不可拆分的短语，用于站内检索；不要放太多定语、日期等描述以降低搜索命中率，不要放多个关键词、不要使用逗号、顿号、斜杠、分号、换行或“和/与/及”等并列词连接多个查询。
